...
<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
                    ...
                    <div class="cart__product">
                            ...
                            <div class="product-remove">
                                <?php
                                    echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
                                        ...
                                        esc_attr( sprintf( __( 'Remove %s from cart', 'brainforward' ), wp_strip_all_tags( $product_name ) ) ),
                                        esc_attr( $product_id ),
                        ...
                        <div class="product__details">
                            <div class="product__price">
                                <span><?php esc_html_e( 'Price', 'brainforward' ); ?>:</span>
                            ...
                            <?php
                                ...
                                if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
                                    echo wp_kses_post( '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'brainforward' ) . '</p>' );
                                }
                            ?>
                            <?php
                            ...
                            <div class="product__subtitle">
                                <span class="m-0"><?php esc_html_e( 'Subtotal', 'brainforward' ); ?>:</span>
    ...
    <div class="mb-4">
        <?php if ( wc_coupons_enabled() ) : ?>
            <div class="coupon">
                <label for="coupon_code" class="screen-reader-text"><?php esc_html_e( 'Coupon:', 'brainforward' ); ?></label>
                <div class="coupon_field">
                    ...
                    <input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Coupon code', 'brainforward' ); ?>" />
                    <button type="submit" class="<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" name="apply_coupon" value="<?php esc_attr_e( 'Apply coupon', 'brainforward' ); ?>"><?php esc_html_e( 'Apply coupon', 'brainforward' ); ?></button>    
                </div>
                <?php do_action( 'woocommerce_cart_coupon' ); ?>
            </div>
        <?php endif; ?>
        <button type="submit" class="d-none update-cart-button button<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'brainforward' ); ?>"><?php esc_html_e( 'Update cart', 'brainforward' ); ?></button>
    </div>
