...
if ( ! class_exists( 'Brainfwd_Functions' ) ) {
    class Brainfwd_Functions {
        ...
        public function __construct() {
            ...
            add_action( 'wp_ajax_brainfwd_send_otp', [ $this, 'send_otp' ] );
            add_action( 'wp_ajax_nopriv_brainfwd_send_otp', [ $this, 'send_otp' ] );
            add_action( 'wp_ajax_brainfwd_verify_otp', [ $this, 'verify_otp' ] );
            add_action( 'wp_ajax_nopriv_brainfwd_verify_otp', [ $this, 'verify_otp' ] );
            add_action( 'wp_ajax_brainfwd_password_login', [ $this, 'password_login' ] );
            add_action( 'wp_ajax_nopriv_brainfwd_password_login', [ $this, 'password_login' ] );
            add_action( 'wp_ajax_brainfwd_forgot_password', [ $this, 'forgot_password' ] );
            add_action( 'wp_ajax_nopriv_brainfwd_forgot_password', [ $this, 'forgot_password' ] );
        ...
        public function after_checkout_billing_form($checkout) {
            ...
            $message = __('Please enter your correct phone number. We will send your Username and Password to this phone number.', 'brainfwd');
        ...
        public function forgot_password() {
            // AJAX nonce যাচাই
            check_ajax_referer('brainfwd_login_nonce', 'nonce');
        ...
        public function password_login() {
            // Check nonce
            if ( empty( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'brainfwd_login_nonce' ) ) {
        ...
        * Add a pingback url auto-discovery header for singularly identifiable articles.
        ...
        public static function page_title() {
            $blog_title = get_theme_mod('blog_list_page_title', __('Blog List', 'brainfwd'));
            $data = '';
            ...
            if (is_home()) {
                ...
                $data .= esc_html__('Event', 'brainfwd');
            } elseif (is_single()) {
                $data .= get_the_title();
            } elseif (is_search()) {
                $data .= __('Search', 'brainfwd') . ' : <span class="search_select">' . esc_html(get_search_query()) . '</span>';
            } elseif (is_archive()) {
                if (class_exists('WooCommerce') && is_shop()) {
                    $data .= woocommerce_page_title(false);
                } elseif ('courses' == get_post_type()) {
                    $data .= esc_html__('Courses', 'brainfwd');
                } else {
                    $data .= get_the_archive_title('', '');
                }
            } elseif (class_exists('WooCommerce') && is_woocommerce()) {
                if (is_shop()) {
                    $data .= esc_html__('Shop Page', 'brainfwd');
                } else {
                    $data .= woocommerce_page_title(false);
                }
            } elseif (is_404()) {
                $data .= esc_html__('Error Page', 'brainfwd');
            } else {
        ...
        public static function get_comment_count() {
            if (!post_password_required() && (comments_open() || get_comments_number()) && get_comments_number() > 0) {
                return get_comments_number_text(
                    esc_html__('No comment', 'brainfwd'),
                    esc_html__('1 Comment', 'brainfwd'),
                    /* translators: %s: number of comments */
                    sprintf(esc_html__('%s Comments', 'brainfwd'), '%')
                );
            }
            return false;
        }
        ...
        public static function get_post_title_array($postType = 'post') {
            ...
            $post_title_array = array('default' => esc_html__('Default', 'brainfwd')) + $post_title_array;
        ...
        public function register_block_styles() {
            register_block_style('core/paragraph', array(
                'name'  => 'highlighted',
                'label' => __('Highlighted', 'brainfwd'),
            ));
            register_block_style('core/button', array(
                'name'  => 'outline',
                'label' => __('Outline', 'brainfwd'),
            ));
        }

        public function register_block_patterns() {
            register_block_pattern('brainforward/hero-section', array(
                'title'   => __('Hero Section', 'brainfwd'),
                'content' => '<!-- wp:group --><div class="wp-block-group"><!-- wp:heading --> <h2>' . __('Your Product Title', 'brainfwd') . '</h2><!-- /wp:heading --></div><!-- /wp:group -->',
            ));
        }
    }
}

new Brainfwd_Functions();
