<?php
if ( ! class_exists( 'Brainfwd_Plugins' ) && is_admin() ) {
    class Brainfwd_Plugins {
        public static $instance;
        public $plugins = array();
        public $menu = 'brainfwd_plugins-install-plugins';
        public $default_path = '';
        ...
        public function __construct() {
            ...
            do_action_ref_array('brainfwd_init', array($this));
        ...
        private function initialize_strings() {
            $this->strings = array(
                'page_title' => __('Install Required Plugins', 'brainfwd'),
                'menu_title' => __('Install Plugins', 'brainfwd'),
                'installing' => __('Installing Plugin: %s', 'brainfwd'),
                'oops' => __('Something went wrong.', 'brainfwd'),
                'notice_can_install_required' => _n_noop('This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'brainfwd'),
                'notice_can_install_recommended' => _n_noop('This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'brainfwd'),
                'notice_cannot_install' => _n_noop('Sorry, but you do not have the correct permissions to install the %s plugin.', 'Sorry, but you do not have the correct permissions to install the %s plugins.', 'brainfwd'),
                'notice_can_activate_required' => _n_noop('The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'brainfwd'),
                'notice_can_activate_recommended' => _n_noop('The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'brainfwd'),
                'notice_cannot_activate' => _n_noop('Sorry, but you do not have the correct permissions to activate the %s plugin.', 'Sorry, but you do not have the correct permissions to activate the %s plugins.', 'brainfwd'),
                'notice_ask_to_update' => _n_noop('The following plugin needs to be updated: %1$s.', 'The following plugins need to be updated: %1$s.', 'brainfwd'),
                'notice_cannot_update' => _n_noop('Sorry, but you do not have the correct permissions to update the %s plugin.', 'Sorry, but you do not have the correct permissions to update the %s plugins.', 'brainfwd'),
                'install_link' => _n_noop('Begin installing plugin', 'Begin installing plugins', 'brainfwd'),
                'activate_link' => _n_noop('Begin activating plugin', 'Begin activating plugins', 'brainfwd'),
                'return' => __('Return to Required Plugins Installer', 'brainfwd'),
                'dashboard' => __('Return to the dashboard', 'brainfwd'),
                'plugin_activated' => __('Plugin activated successfully.', 'brainfwd'),
                'activated_successfully' => __('The following plugin was activated successfully:', 'brainfwd'),
                'complete' => __('All plugins installed and activated successfully. %1$s', 'brainfwd'),
                'dismiss' => __('Dismiss this notice', 'brainfwd'),
            );
        }
        ...
        public function thickbox() {
            ...
            if ( ! get_user_meta( get_current_user_id(), 'brainfwd_dismissed_notice', true ) ) {
                add_thickbox();
            }
        ...
        public function install_plugins_page() {
            ...
            $plugin_table = new Brainfwd_List_Table;

            // Sanitize and validate the 'action' parameter from POST.
            $action = isset($_POST['action']) ? sanitize_text_field($_POST['action']) : '';

            // Return early if processing a plugin installation action.
            if ( 'brainfwd_plugins-bulk-install' === $action && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            }

            ?>
            <div class="brainfwd_plugins wrap">
                <?php if ( version_compare( $this->wp_version, '3.8', '<' ) ) : ?>
                    <div class="notice notice-warning">
                        <p><?php _e( 'This theme requires WordPress version 3.8 or higher. Please upgrade your WordPress installation.', 'brainfwd' ); ?></p>
                    </div>
                <?php endif; ?>
                ...
                <form id="brainfwd_plugins-plugins" action="" method="post">
                    <input type="hidden" name="brainfwd_plugins-page" value="<?php echo esc_attr($this->menu); ?>" />
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['brainfwd_plugins-install'] ) && 'install-plugin' == $_GET['brainfwd_plugins-install'] ) ) {
                check_admin_referer( 'brainfwd_plugins-install' );
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            ...
                            'brainfwd_plugins-install' => 'install-plugin',
                        ),
                        network_admin_url( 'themes.php' )
                    ),
                    'brainfwd_plugins-install'
                );
        
                // Direct file operations instead of using WP_Filesystem
                if ( false === ( $creds = request_filesystem_credentials( esc_url_raw( $url ), '', false, false, array( 'brainfwd_plugins-install' ) ) ) ) {
                    return true;
                }
        
                // If credentials are provided, proceed with file operations
                if ( ! $creds ) {
                    request_filesystem_credentials( esc_url_raw( $url ), '', true, false, array( 'brainfwd_plugins-install' ) );
                    return true;
                }
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'brainfwd' ) . '">' . esc_html__( 'Return to the Dashboard', 'brainfwd' ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['brainfwd_plugins-activate'] ) && 'activate-plugin' == $_GET['brainfwd_plugins-activate'] ) ) {
                check_admin_referer( 'brainfwd_plugins-activate', 'brainfwd_plugins-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'brainfwd' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'brainfwd' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'brainfwd' ) . '</a>'  : '';

                // Define all of the action links.
                $action_links = apply_filters(
                    'brainfwd_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . esc_url( add_query_arg( 'brainfwd_plugins-dismiss', 'dismiss_admin_notices' ) ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'brainfwd', 'brainfwd', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'brainfwd', 'brainfwd', $rendered, $nag_class );
                }
            }

            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'brainfwd' );
            }
        ...
        public function dismiss() {
            ...
            if ( isset( $_GET['brainfwd_plugins-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'brainfwd_dismissed_notice', 1 );
            }
        ...
        protected function is_brainfwd_page() {
        ...
        public function update_dismiss() {
            ...
            delete_user_meta( get_current_user_id(), 'brainfwd_dismissed_notice' );
        ...
        public static function get_instance() {
            ...
            if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Brainfwd_Plugins ) ) {
                self::$instance = new Brainfwd_Plugins();
            }
    ...
    $brainfwd_plugins = Brainfwd_Plugins::get_instance();

}

if ( ! function_exists( 'brainfwd_plugins' ) ) {
    function brainfwd_plugins( $plugins, $config = array() ) {
        foreach ( $plugins as $plugin ) {
            Brainfwd_Plugins::$instance->register( $plugin );
        }
        if ( $config ) {
            Brainfwd_Plugins::$instance->config( $config );
        }
    }
}
...
if ( ! class_exists( 'Brainfwd_List_Table' ) ) {
    
    class Brainfwd_List_Table extends WP_List_Table {
        ...
        protected function _gather_plugin_data() {
            ...
            Brainfwd_Plugins::$instance->admin_init();
            Brainfwd_Plugins::$instance->thickbox();
            ...
            foreach ( Brainfwd_Plugins::$instance->plugins as $plugin ) {
                if ( is_plugin_active_custom( $plugin['file_path'] ) ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'brainfwd' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'brainfwd' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'brainfwd' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'brainfwd' ) : __( 'Recommended', 'brainfwd' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'brainfwd' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'brainfwd' ) );
                }
        ...
        protected function _get_plugin_data_from_name( $name, $data = 'slug' ) {
            ...
            foreach ( Brainfwd_Plugins::$instance->plugins as $plugin => $values ) {
                if ( $name == $values['name'] && isset( $values[$data] ) ) {
                    return $values[$data];
                }
            }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Install', 'brainfwd' ) . ' %2$s">' . __( 'Install', 'brainfwd' ) . '</a>',
                        esc_url(
						    wp_nonce_url(
                                add_query_arg(
                                    array(
                                        'page'          => urlencode( Brainfwd_Plugins::$instance->menu ),
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Activate', 'brainfwd' ) . ' %2$s">' . __( 'Activate', 'brainfwd' ) . '</a>',
                        esc_url(
                            add_query_arg(
                                array(
                                    'page'                 => urlencode( Brainfwd_Plugins::$instance->menu ),
                                    ...
                                    'plugin_name'          => urlencode( $item['sanitized_plugin'] ),
                                    'plugin_source'        => urlencode( $item['url'] ),
                                    'brainfwd_plugins-activate'       => 'activate-plugin',
                                    'brainfwd_plugins-activate-nonce' => urlencode( wp_create_nonce( 'brainfwd_plugins-activate' ) ),
                                ),
                                network_admin_url( 'themes.php' )
                            )
                        ),
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'brainfwd' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'brainfwd' ),
                'source' => __( 'Source', 'brainfwd' ),
                'type'   => __( 'Type', 'brainfwd' ),
                'status' => __( 'Status', 'brainfwd' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'brainfwd_plugins-bulk-install'  => __( 'Install', 'brainfwd' ),
                'brainfwd_plugins-bulk-activate' => __( 'Activate', 'brainfwd' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'brainfwd_plugins-bulk-install' === $this->current_action() ) {
                ...
                $url = wp_nonce_url(
                    add_query_arg(
                        array(
                            'page'          => urlencode( Brainfwd_Plugins::$instance->menu ),
                            'brainfwd_plugins-action'  => 'install-selected',
                ...
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'brainfwd_plugins-empty';
                    $i++;
                }
        
                if ( is_wp_error( $api ) ) {
                    wp_die( Brainfwd_Plugins::$instance->strings['oops'] . var_dump( $api ) );
                }
                ...
                $url   = esc_url_raw( add_query_arg( array( 'page' => urlencode( Brainfwd_Plugins::$instance->menu ) ), network_admin_url( 'themes.php' ) ) );
                $nonce = 'bulk-plugins';
                $names = $plugin_names;
        
                $installer = new TGM_Bulk_Installer( $skin = new TGM_Bulk_Installer_Skin( compact( 'url', 'nonce', 'names' ) ) );
        
                echo '<div class="brainfwd_plugins wrap">';
                    if ( version_compare( Brainfwd_Plugins::$instance->wp_version, '3.8', '<' ) ) {
                        echo '<div class="notice notice-warning"><p>';
                        _e( 'This theme requires WordPress version 3.8 or higher. Please upgrade your WordPress installation.', 'brainfwd' );
                        echo '</p></div>';
                    }
            ...
            if ( 'brainfwd_plugins-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'brainfwd' ), $imploded );
                }
...
if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && Brainfwd_Plugins::$instance->menu === $_GET['page'] ) ) {
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
            ...
            public function bulk_install( $packages ) {
                ...
                if ( Brainfwd_Plugins::$instance->is_automatic ) {
            ...
            public function run( $options ) {
                ...
                if ( Brainfwd_Plugins::$instance->is_automatic ) {
                    ...
                    Brainfwd_Plugins::$instance->populate_file_path();
            ...
            public function install_strings() {
                ...
                $this->strings['no_package']          = __( 'Install package not available.', 'brainfwd' );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'brainfwd' );
                $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'brainfwd' );
                $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'brainfwd' );
                $this->strings['process_failed']      = __( 'Plugin install failed.', 'brainfwd' );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', 'brainfwd' );

            }
            public function activate_strings() {

                $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'brainfwd' );
                $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'brainfwd' );
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( Brainfwd_Plugins::$instance->is_automatic ) {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'brainfwd' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'brainfwd' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'brainfwd' ) . '</span><span class="hidden">' . __( 'Hide Details', 'brainfwd' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'brainfwd' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'brainfwd' );
                }
                // Default installation strings.
                else {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'brainfwd' );
                    $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'brainfwd' );
                    $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'brainfwd' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'brainfwd' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'brainfwd' ) . '</span><span class="hidden">' . __( 'Hide Details', 'brainfwd' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'brainfwd' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'brainfwd' );
                }
            ...
            public function bulk_footer() {
                ...
                foreach ( Brainfwd_Plugins::$instance->plugins as $plugin ) {
                    if ( ! is_plugin_active_custom( $plugin['file_path'] ) ) {
                        echo '<p><a href="' . esc_url( add_query_arg( 'page', urlencode( Brainfwd_Plugins::$instance->menu ), network_admin_url( 'themes.php' ) ) ) . '" title="' . esc_attr( Brainfwd_Plugins::$instance->strings['return'] ) . '" target="_parent">' . Brainfwd_Plugins::$instance->strings['return'] . '</a></p>';
                        $complete[] = $plugin;
                        break;
                    }
                    // Nothing to store.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( Brainfwd_Plugins::$instance->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'brainfwd' ) . '">' . __( 'Return to the Dashboard', 'brainfwd' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
...
function brainfwd_register_required_plugins() {
    $plugins = array(
        // array(
        //     'name'      => 'Brainforward',
        //     'slug'      => 'brainfwd',
    ...
    $config = array(
        'id'           => 'brainfwd',
        'default_path' => '',
        'menu'         => 'brainfwd_plugins-install-plugins',
        'has_notices'  => true,
        'dismissable'  => true,
        'dismiss_msg'  => '',
        'is_automatic' => false,
    ...
    brainfwd_plugins($plugins, $config);
}


add_action('brainfwd_register', 'brainfwd_register_required_plugins');