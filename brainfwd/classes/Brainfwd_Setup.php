<?php
// Check if the class Brainfwd_Setup doesn't already exist to avoid redeclaration
if ( ! class_exists( 'Brainfwd_Setup' ) ) {
    // Define the Brainfwd_Setup class
    class Brainfwd_Setup {
        // Constructor method to initialize the class
        public function __construct() {
            // Hook the setup_theme method to the after_setup_theme action
            add_action( 'after_setup_theme', array( $this, 'setup_theme' ) );
            add_action('init', [$this, 'brainfwd_load_textdomain']);
        }

        public function brainfwd_load_textdomain() {
            // Load the theme's translation files
            load_theme_textdomain( 'brainfwd', BRAINFWD_LANG_DIR );
        }
        ...
        public function setup_theme() {
            ...
            register_nav_menus( array(
                'primary_menu' => esc_html__( 'Primary Menu', 'brainfwd' )
            ) );
            ...
            $GLOBALS['content_width'] = apply_filters( 'brainfwd_content_width', 850 );
        }
    ...
    // Instantiate the Brainfwd_Setup class to initialize the theme setup
    new Brainfwd_Setup();
}
