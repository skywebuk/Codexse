...
class WooCommerce_Functions {
    public function __construct() {
        ...
        add_filter( 'comment_form_defaults', [$this, 'brainfwd_change_woocommerce_review_submit_button'] );

        add_filter( 'woocommerce_checkout_fields', [ $this, 'brainfwd_checkout_fields' ] );
    ...
    public function archive_add_to_cart_text( $text, $product ) {
        if ( $product->is_type( 'simple' ) ) {
            return __( 'Buy Now', 'brainfwd' );
        }
        if ( $product->is_type( 'variable' ) ) {
            return __( 'Choose Options', 'brainfwd' );
        }
        if ( $product->is_type( 'external' ) ) {
            return __( 'View Product', 'brainfwd' );
        }
        if ( $product->is_type( 'grouped' ) ) {
            return __( 'View Group', 'brainfwd' );
        }
    ...
    public function single_add_to_cart_text( $text, $product ) {
        if ( $product->is_type( 'simple' ) ) {
            return __( 'Purchase Now', 'brainfwd' );
        }
        if ( $product->is_type( 'variable' ) ) {
            return __( 'Select Your Option', 'brainfwd' );
        }
        if ( $product->is_type( 'external' ) ) {
            return __( 'Go to Shop', 'brainfwd' );
        }
        if ( $product->is_type( 'grouped' ) ) {
            return __( 'View Grouped Product', 'brainfwd' );
        }
        return $text; // fallback
    }
    ...
    public function loop_add_to_cart_link( $html, $product ) {
        ...
        return str_replace( 'Add to cart', __( 'Shop Now', 'brainfwd' ), $html );
    }



    public function custom_place_order_button_text( $button_text ) {
        return __( 'Confirm Order', 'brainfwd' ); // Change 'Confirm Order' to your desired text
    }
    ...
    public function custom_terms_and_conditions_text( $text ) {
        ...
        $terms_link  = '<a href="' . $terms_url . '" class="woocommerce-terms-and-conditions-link" target="_blank">' . __( 'Terms and Conditions', 'brainfwd' ) . '</a>';
        $refund_link = '<a href="' . $refund_url . '" target="_blank">' . __( 'Refund Policy', 'brainfwd' ) . '</a>';

        $text = sprintf(
            __( 'I have read and agree to the website %1$s and %2$s.', 'brainfwd' ),
            $terms_link,
            $refund_link
        );
	...
	public function brainfwd_checkout_fields( $fields ) {
		$fields['billing'] = [];
		...
		$fields['billing']['billing_email'] = [
			'label'       => __('Email Address', 'brainfwd'),
			'required'    => true,
			'class'       => ['form-row-wide'],
			'clear'       => true,
		...
		$fields['billing']['billing_phone'] = [
			'label'       => __('Phone Number', 'brainfwd'),
			'required'    => true,
			'class'       => ['form-row-wide'],
			'clear'       => true,
    ...
    public function brainfwd_change_woocommerce_review_submit_button( $defaults ) {
    ...
    public function update_mini_cart() {
        ...
        check_ajax_referer('brainfwd_mini_cart_nonce', 'nonce');
