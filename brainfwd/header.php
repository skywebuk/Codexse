...
<body <?php body_class('overflow-hidden'); ?> >
    ...
    <!-- Preloader Start -->
    <?php
    $loader_visibility = get_theme_mod('brainfwd_loader_visibility', 'show');
    $loader_type = get_theme_mod('brainfwd_loader_type', 'loader');
    $loader_text = get_theme_mod('loader_text_setting', __('Loading', 'brainfwd'));
    $loader_image  = get_theme_mod('brainfwd_loader_image', get_theme_file_uri('assets/images/loader.gif'));
    $offer = get_theme_mod('navbar_offer_setting', 'hide');
    $offer_text = get_theme_mod('navbar_offer_text', __('🧠 Special Offer: Get 50% off your first course - Enroll today!', 'brainfwd'));
                ...
                <div class="loader">
                    <svg class="loader__svg">
                        <text class="loader__text" text-anchor="middle" x="50%" y="75%">
                            <?php echo esc_html(substr($loader_text, 0, 10)); ?>
                        </text>
                    </svg>
                </div>
            </div>
	...
	<main class="main_wrapper">
        <?php if ($offer !== 'hide' && !empty($offer_text)) : ?>
            <div class="alert__bar">
                ...
                <button type="button" class="alert__close" aria-label="<?php esc_attr_e('Close alert', 'brainfwd'); ?>">
