...
$format_label = $post_format ? get_post_format_string($post_format) : __('Standard', 'brainfwd');
?>
    ...
    <div class="single_post-thumbnail__overlay">
        <div class="single_post-thumbnail__meta">
            ...
            <ul class="post-card__meta meta-list">
                ...
                <?php endif; ?>
                <?php if ($date !== 'hide'): ?>
                    <li class="post-meta__date"><i class="ri-calendar-line icon"></i><?php echo wp_kses_post(Brainfwd_Functions::get_post_date()); ?></li>
                <?php endif; ?>
                <?php if ($comment !== 'hide' && !empty(Brainfwd_Functions::get_comment_count())): ?>
                    <li class="post-meta__comments"><i class="ri-message-2-line icon"></i><?php echo wp_kses_post(Brainfwd_Functions::get_comment_count()); ?></li>
                <?php endif; ?>
                    ...
                    <div class="post-details__wrapper">
                        <article <?php post_class(['post-single', get_post_type()]); ?>>
                            <div class="post-body">
                                <div class="post-content">
                                    ...
                                    the_content(
                                        sprintf(
                                            esc_html__('Continue reading %s', 'brainfwd'),
                                            '<span class="screen-reader-text">' . get_the_title() . '</span>'
                                        )
                                    );

                                    wp_link_pages([
                                        'before'           => '<div class="page-links"><span class="page-links-title">' . esc_html__('Pages:', 'brainfwd') . '</span>',
                                        'after'            => '</div>',
                        ...
                        <?php if ($navigation !== 'hide'):
                            ...
                            if ($prev_post || $next_post): ?>
                                <nav class="post-navigation d-flex justify-content-between mt-5" aria-label="<?php esc_attr_e('Post navigation', 'brainfwd'); ?>">
                                    <div class="post-navigation__prev">
                                        <?php
                                        if ($prev_post) {
                                            previous_post_link(
                                                '%link',
                                                '<div class="button-content"><span class="label"><i class="ri-arrow-left-line me-2"></i>' . esc_html__('Prev Post', 'brainfwd') . '</span><h4 class="button-title">%title</h4></div>'
                                            );
                                        }
                                        ?>
                                    </div>
                                    <div class="post-navigation__next">
                                        ...
                                        if ($next_post) {
                                            next_post_link(
                                                '%link',
                                                '<div class="button-content"><span class="label">' . esc_html__('Next Post', 'brainfwd') . ' <i class="ri-arrow-right-line ms-2"></i></span><h4 class="button-title">%title</h4></div>'
                                            );
                                        }
                                        ?>
                                    </div>
