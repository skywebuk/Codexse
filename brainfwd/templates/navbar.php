...
$button_text    = get_theme_mod('navbar_button_text_setting', __('Get Started', 'brainfwd'));
...
$light_logo     = get_theme_mod('brainfwd_light_logo');
...
if (class_exists('WooCommerce') && $cart !== 'hide') {
    ...
    // Cart toggle button
    $nav_actions .= '<button class="nav_action cart_toggle" title="' . esc_attr__('View your shopping cart', 'brainfwd') . '">';
...
if (is_active_sidebar('navbar_toggle_sidebar') && $sidebar !== 'hide') {
    ...
    echo '<div class="navbar__sidebar-header">';
        ...
        echo '</div>';
        echo '<button class="navbar__sidebar-close" type="button" aria-label="' . esc_attr__('Close sidebar', 'brainfwd') . '">';
        ...
        echo '<div class="container">';
            echo '<div class="navbar__search-header">';
                echo '<div class="navbar__search-logo">';
                    echo wp_kses_post($logo);
                echo '</div>';
                echo '<button class="navbar__search-close" type="button" aria-label="' . esc_attr__('Close search', 'brainfwd') . '">';
                    echo '<i class="ri-close-large-line"></i>';
                echo '</button>';
            echo '</div>';
            echo '<div class="navbar__search-body">';
                echo '<form role="search" method="get" class="navbar__search-form" action="' . esc_url(home_url('/')) . '">';
                    echo '<input type="search" class="navbar__search-input" placeholder="' . esc_attr__('Search here...', 'brainfwd') . '" value="' . esc_attr(get_search_query()) . '" name="s" />';
                    echo '<button type="submit" class="navbar__search-submit" aria-label="' . esc_attr__('Submit search', 'brainfwd') . '">';
        ...
        <div class="navbar__row">
            ...
            <div class="navbar__menu" id="nav_menu">
                ...
                wp_nav_menu([
                    ...
                    'menu_class'     => 'nav',
                    'container'      => '',
                    'fallback_cb'    => 'brainfwd_mainmenu_demo_content',
                    'walker'         => new Brainfwd_Menu_Walker,
                ]);
                ?>
